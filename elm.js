...
{
	if (region.ak.U === region.at.U)
	{
		return 'on line ' + region.ak.U;
	}
	return 'on lines ' + region.ak.U + ' through ' + region.at.U;
}
...
var author$project$Main$intervalSize = 1;
...
var author$project$Main$pointsToTuple = function (points) {
	return A2(
		elm$core$List$map,
		function (n) {
			return _Utils_Tuple2(n.ab, n.ac);
		},
		points);
};
...
var mdgriffith$elm_style_animation$Animation$initMotion = F2(
	function (position, unit) {
		return {
			I: mdgriffith$elm_style_animation$Animation$Model$Spring(
				{ar: 26, aN: 170}),
...
var mdgriffith$elm_style_animation$Animation$initialState = function (current) {
	return {
		...
		aQ: {
			...
			aW: elm$time$Time$millisToPosix(0)
		}
	};
};
...
var mdgriffith$elm_style_animation$Animation$defaultInterpolationByProperty = function (prop) {
	var linear = function (duration) {
		return mdgriffith$elm_style_animation$Animation$Model$Easing(
			{ae: duration, af: elm$core$Basics$identity, aG: 1, ak: 0});
	};
		...
		switch (cmd.$) {
			...
			case 7:
				...
				return mdgriffith$elm_style_animation$Animation$Model$VerticalTo(
					fn(motion));
			case 8:
				var control1 = cmd.a.Q;
				var control2 = cmd.a.R;
				var point = cmd.a.p;
...
var author$project$Main$init = function (_n0) {
	var model = {
		T: elm$core$Basics$sin,
		z: 1.0,
		B: 100.0,
		r: mdgriffith$elm_style_animation$Animation$style(
			_List_fromArray(
				[
					mdgriffith$elm_style_animation$Animation$points(
					author$project$Main$pointsToTuple(
...
var elm$browser$Browser$AnimationManager$State = F3(
	function (subs, request, oldTime) {
		return {ai: oldTime, aL: request, aO: subs};
	});
...
var elm$url$Url$Url = F6(
	function (protocol, host, port_, path, query, fragment) {
		return {av: fragment, aw: host, aA: path, aD: port_, aH: protocol, aI: query};
	});
...
var author$project$Main$addScalarsToFunction = function (model) {
	return function (n) {
		return A3(
			...
			(model.B * model.T(
				elm$core$Basics$degrees(model.z) * n)) + (author$project$Main$graphHeight / 2));
	};
};
...
var mdgriffith$elm_style_animation$Animation$Model$refreshTiming = F2(
	function (now, timing) {
		var dt = elm$time$Time$posixToMillis(now) - elm$time$Time$posixToMillis(timing.aq);
		return {
			aq: now,
			aW: ((dt > 34) || (!elm$time$Time$posixToMillis(timing.aq))) ? elm$time$Time$millisToPosix(
...
var mdgriffith$elm_style_animation$Animation$Model$isCmdDone = function (cmd) {
	var motionDone = function (motion) {
		return (!motion.be) && _Utils_eq(motion.a6, motion.a9);
	};
	switch (cmd.$) {
		case 0:
			var m1 = cmd.a;
		...
		case 16:
			var arc = cmd.a;
			return motionDone(arc.ab) && (motionDone(arc.ac) && (motionDone(arc.V) && (motionDone(arc.X) && motionDone(arc.S))));
		case 17:
			var arc = cmd.a;
			return motionDone(arc.ab) && (motionDone(arc.ac) && (motionDone(arc.V) && (motionDone(arc.X) && motionDone(arc.S))));
		default:
...
var mdgriffith$elm_style_animation$Animation$Model$isDone = function (property) {
	var motionDone = function (motion) {
		var runningInterpolation = A2(elm$core$Maybe$withDefault, motion.I, motion.a1);
		switch (runningInterpolation.$) {
			case 0:
				return (!motion.be) && _Utils_eq(motion.a6, motion.a9);
			case 1:
				var eased = runningInterpolation.a;
				return (eased.aG === 1) || ((!eased.aG) && _Utils_eq(motion.a6, motion.a9));
			default:
				var speed = runningInterpolation.a;
				return _Utils_eq(motion.a6, motion.a9);
		}
	};
	switch (property.$) {
		case 0:
			...
			function (motion, targetMotion) {
				...
				if (_n27.$ === 1) {
					...
					return _Utils_update(
						motion,
						{
							I: mdgriffith$elm_style_animation$Animation$Model$Easing(
								_Utils_update(
									ease,
									{ak: motion.a6})),
							a9: targetMotion.a6
					...
					return mdgriffith$elm_style_animation$Animation$Model$ClockwiseArc(
						function () {
							...
							return _Utils_update(
								arc,
								{
									S: A2(setMotionTarget, endAngle, target.S),
									V: A2(setMotionTarget, radius, target.V),
									X: A2(setMotionTarget, startAngle, target.X),
									ab: A2(setMotionTarget, x, target.ab),
									ac: A2(setMotionTarget, y, target.ac)
								});
						}());
				} else {
					return cmd;
		...
		var setMotionTarget = F2(
			function (motion, targetMotion) {
				var newMotion = overrideInterpolation ? _Utils_update(
					motion,
					{
						a1: elm$core$Maybe$Just(targetMotion.I)
					}) : motion;
				var _n13 = newMotion.a1;
				if (_n13.$ === 1) {
					var _n14 = newMotion.I;
					if (_n14.$ === 1) {
						var ease = _n14.a;
						return _Utils_update(
							newMotion,
...
var mdgriffith$elm_style_animation$Animation$Model$stepInterpolation = F2(
	function (posix, motion) {
		var interpolationToUse = A2(elm$core$Maybe$withDefault, motion.I, motion.a1);
		var dtms = elm$time$Time$posixToMillis(posix);
		switch (interpolationToUse.$) {
			case 2:
				var perSecond = interpolationToUse.a.aB;
				var _n1 = function () {
					if (_Utils_cmp(motion.a6, motion.a9) < 0) {
						var _new = motion.a6 + (perSecond * (dtms / 1000));
						return _Utils_Tuple2(
							_new,
							_Utils_cmp(_new, motion.a9) > -1);
					} else {
						var _new = motion.a6 - (perSecond * (dtms / 1000));
						return _Utils_Tuple2(
							_new,
				...
				return finished ? _Utils_update(
					motion,
					{a6: motion.a9, be: 0.0}) : _Utils_update(
					motion,
					{a6: newPos, be: perSecond * 1000});
			case 0:
				var stiffness = interpolationToUse.a.aN;
				var damping = interpolationToUse.a.ar;
				var fspring = stiffness * (motion.a9 - motion.a6);
				var fdamper = ((-1) * damping) * motion.be;
				var dt = dtms / 1000;
				var a = fspring + fdamper;
				var newVelocity = motion.be + (a * dt);
				var newPos = motion.a6 + (newVelocity * dt);
				var dx = elm$core$Basics$abs(motion.a9 - newPos);
				return ((_Utils_cmp(dx, mdgriffith$elm_style_animation$Animation$Model$tolerance) < 0) && (_Utils_cmp(
					...
					{a6: motion.a9, be: 0.0}) : _Utils_update(
					motion,
					{a6: newPos, be: newVelocity});
			default:
				var progress = interpolationToUse.a.aG;
				var duration = interpolationToUse.a.ae;
				var ease = interpolationToUse.a.af;
				var start = interpolationToUse.a.ak;
				...
				var distance = motion.a9 - start;
				var newPos = ((((eased * distance) + start) * 10000) | 0) / 10000;
				var newVelocity = (newProgress === 1) ? 0 : ((newPos - motion.a6) / dtms);
				var _n2 = motion.a1;
				if (_n2.$ === 1) {
					return _Utils_update(
						motion,
						{
							I: mdgriffith$elm_style_animation$Animation$Model$Easing(
								{ae: duration, af: ease, aG: newProgress, ak: start}),
							a6: newPos,
							be: newVelocity
						});
				} else {
...
var mdgriffith$elm_style_animation$Animation$Model$updateAnimation = F2(
	function (_n0, _n1) {
		var now = _n0;
		var model = _n1;
		var timing = A2(mdgriffith$elm_style_animation$Animation$Model$refreshTiming, now, model.aQ);
		var _n2 = A2(
			elm$core$List$partition,
			function (_n4) {
				var wait = _n4.a;
			...
			A2(
				...
				function (_n3) {
					...
					return _Utils_Tuple2(
						elm$time$Time$millisToPosix(
							elm$time$Time$posixToMillis(wait) - elm$time$Time$posixToMillis(timing.aW)),
						mySteps);
				},
				model.ag));
		...
		var _n5 = function () {
			...
			if (!_n6.$) {
				...
				return _Utils_Tuple2(model.al, model.r);
			}
		}();
		var steps = _n5.a;
		var style = _n5.b;
		var _n8 = A3(mdgriffith$elm_style_animation$Animation$Model$resolveSteps, style, steps, timing.aW);
			...
			case 0:
				...
				return _Utils_Tuple2(
					_Utils_update(
						...
						{
							z: function () {
								var _n1 = elm$core$String$toFloat(changeData);
								if (_n1.$ === 1) {
									return 0;
								} else {
									var val = _n1.a;
			...
			case 1:
				...
				return _Utils_Tuple2(
					_Utils_update(
						...
						{
							B: function () {
								var _n2 = elm$core$String$toFloat(changeData);
								if (_n2.$ === 1) {
									return 0;
								} else {
									var val = _n2.a;
			...
			default:
				...
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{
							r: A2(mdgriffith$elm_style_animation$Animation$update, animMsg, model.r)
						}),
					elm$core$Platform$Cmd$none);
		}
	});
...
var author$project$Main$FadeLines = {$: 3};
		...
		return A2(
			...
			A2(
				...
				function (_n11) {
					var x = _n11.a;
					var y = _n11.b;
					return elm$core$String$fromFloat(x.a6) + (',' + elm$core$String$fromFloat(y.a6));
				},
				coords));
	};
	switch (cmd.$) {
		case 0:
			var x = cmd.a;
			var y = cmd.b;
			return 'm ' + (elm$core$String$fromFloat(x.a6) + (',' + elm$core$String$fromFloat(y.a6)));
		case 1:
			var x = cmd.a;
			var y = cmd.b;
			return 'M ' + (elm$core$String$fromFloat(x.a6) + (',' + elm$core$String$fromFloat(y.a6)));
		case 2:
			var x = cmd.a;
			var y = cmd.b;
			return 'l ' + (elm$core$String$fromFloat(x.a6) + (',' + elm$core$String$fromFloat(y.a6)));
		case 3:
			var x = cmd.a;
			var y = cmd.b;
			return 'L ' + (elm$core$String$fromFloat(x.a6) + (',' + elm$core$String$fromFloat(y.a6)));
		case 4:
			var a = cmd.a;
			return 'h ' + elm$core$String$fromFloat(a.a6);
		case 5:
			var a = cmd.a;
			return 'H ' + elm$core$String$fromFloat(a.a6);
		case 6:
			var a = cmd.a;
			return 'v ' + elm$core$String$fromFloat(a.a6);
		case 7:
			var a = cmd.a;
			return 'V ' + elm$core$String$fromFloat(a.a6);
		case 8:
			...
			var c1x = _n3.a;
			var c1y = _n3.b;
			return 'c ' + (elm$core$String$fromFloat(c1x.a6) + (' ' + (elm$core$String$fromFloat(c1y.a6) + (', ' + (elm$core$String$fromFloat(c2x.a6) + (' ' + (elm$core$String$fromFloat(c2y.a6) + (', ' + (elm$core$String$fromFloat(p1x.a6) + (' ' + elm$core$String$fromFloat(p1y.a6)))))))))));
		case 9:
			var control1 = cmd.a.Q;
			var control2 = cmd.a.R;
			var point = cmd.a.p;
		...
		case 10:
			...
			var c1x = _n8.a;
			var c1y = _n8.b;
			return 'q ' + (elm$core$String$fromFloat(c1x.a6) + (' ' + (elm$core$String$fromFloat(c1y.a6) + (', ' + (elm$core$String$fromFloat(p1x.a6) + (' ' + elm$core$String$fromFloat(p1y.a6)))))));
		case 11:
			var control = cmd.a.P;
			var point = cmd.a.p;
			var _n9 = point;
		...
		case 16:
			var arc = cmd.a;
			var deltaAngle = arc.S.a6 - arc.X.a6;
			if (_Utils_cmp(deltaAngle, 360 - 1.0e-6) > 0) {
				var dy = arc.V.a6 * elm$core$Basics$sin(
					elm$core$Basics$degrees(arc.X.a6));
				var dx = arc.V.a6 * elm$core$Basics$cos(
					elm$core$Basics$degrees(arc.X.a6));
				return 'A ' + (elm$core$String$fromFloat(arc.V.a6) + (',' + (elm$core$String$fromFloat(arc.V.a6) + (',0,1,1,' + (elm$core$String$fromFloat(arc.ab.a6 - dx) + (',' + (elm$core$String$fromFloat(arc.ac.a6 - dy) + (' A ' + (elm$core$String$fromFloat(arc.V.a6) + (',' + (elm$core$String$fromFloat(arc.V.a6) + (',0,1,1,' + (elm$core$String$fromFloat(arc.ab.a6 + dx) + (',' + elm$core$String$fromFloat(arc.ac.a6 + dy)))))))))))))));
			} else {
				return 'A ' + (elm$core$String$fromFloat(arc.V.a6) + (',' + (elm$core$String$fromFloat(arc.V.a6) + (' 0 ' + (((deltaAngle >= 180) ? '1' : '0') + (' ' + ('1' + (' ' + (elm$core$String$fromFloat(
					arc.ab.a6 + (arc.V.a6 * elm$core$Basics$cos(
						elm$core$Basics$degrees(arc.S.a6)))) + (',' + elm$core$String$fromFloat(
					arc.ac.a6 + (arc.V.a6 * elm$core$Basics$sin(
						elm$core$Basics$degrees(arc.S.a6))))))))))))));
			}
		case 17:
			var arc = cmd.a;
			var deltaAngle = arc.S.a6 - arc.X.a6;
			if (_Utils_cmp(deltaAngle, 360 - 1.0e-6) > 0) {
				...
				var dx = arc.V.a6 * elm$core$Basics$cos(
					elm$core$Basics$degrees(arc.X.a6));
				return 'A ' + (elm$core$String$fromFloat(arc.V.a6) + (',' + (elm$core$String$fromFloat(arc.V.a6) + (',0,1,0,' + (elm$core$String$fromFloat(arc.ab.a6 - dx) + (',' + (elm$core$String$fromFloat(arc.ac.a6 - dy) + (' A ' + (elm$core$String$fromFloat(arc.V.a6) + (',' + (elm$core$String$fromFloat(arc.V.a6) + (',0,1,1,' + (elm$core$String$fromFloat(arc.ab.a6 + dx) + (',' + elm$core$String$fromFloat(arc.ac.a6 + dy)))))))))))))));
			} else {
				return 'A ' + (elm$core$String$fromFloat(arc.V.a6) + (',' + (elm$core$String$fromFloat(arc.V.a6) + (' 0 ' + ((((arc.X.a6 - arc.S.a6) >= 180) ? '1' : '0') + (' ' + ('0' + (' ' + (elm$core$String$fromFloat(
					arc.ab.a6 + (arc.V.a6 * elm$core$Basics$cos(arc.S.a6))) + (',' + elm$core$String$fromFloat(
					arc.ac.a6 + (arc.V.a6 * elm$core$Basics$sin(arc.S.a6)))))))))))));
			}
		default:
			return 'z';
	}
		...
		switch (prop.$) {
			...
			case 1:
				...
				return 'rgba(' + (elm$core$String$fromInt(
					elm$core$Basics$round(r.a6)) + (',' + (elm$core$String$fromInt(
					elm$core$Basics$round(g.a6)) + (',' + (elm$core$String$fromInt(
					elm$core$Basics$round(b.a6)) + (',' + (elm$core$String$fromFloat(a.a6) + ')')))))));
			case 2:
				var name = prop.a;
				var inset = prop.b;
				var shadow = prop.c;
				return (inset ? 'inset ' : '') + (elm$core$String$fromFloat(shadow.m.a6) + ('px' + (' ' + (elm$core$String$fromFloat(shadow.n.a6) + ('px' + (' ' + (elm$core$String$fromFloat(shadow.l.a6) + ('px' + (' ' + ((((name === 'text-shadow') || (name === 'drop-shadow')) ? '' : (elm$core$String$fromFloat(shadow.q.a6) + ('px' + ' '))) + ('rgba(' + (elm$core$String$fromInt(
					elm$core$Basics$round(shadow.k.a6)) + (', ' + (elm$core$String$fromInt(
					elm$core$Basics$round(shadow.j.a6)) + (', ' + (elm$core$String$fromInt(
					elm$core$Basics$round(shadow.h.a6)) + (', ' + (elm$core$String$fromFloat(shadow.g.a6) + ')'))))))))))))))))));
			case 3:
				var x = prop.b;
				return _Utils_ap(
					elm$core$String$fromFloat(x.a6),
					x.bb);
			case 4:
				var x = prop.b;
				var y = prop.c;
				return _Utils_ap(
					...
					_Utils_ap(
						...
						_Utils_ap(
							delim,
							_Utils_ap(
								elm$core$String$fromFloat(y.a6),
								y.bb))));
			case 5:
				var x = prop.b;
				var y = prop.c;
							...
							_Utils_ap(
								...
								_Utils_ap(
									...
									_Utils_ap(
										delim,
										_Utils_ap(
											elm$core$String$fromFloat(z.a6),
											z.bb)))))));
			case 6:
				var w = prop.b;
				var x = prop.c;
				...
				var z = prop.e;
				return _Utils_ap(
					elm$core$String$fromFloat(w.a6),
					_Utils_ap(
						w.bb,
						_Utils_ap(
							delim,
...
var mdgriffith$elm_style_animation$Animation$Render$render3dRotation = function (prop) {
	if (prop.$ === 5) {
		...
		var y = prop.c;
		var z = prop.d;
		return 'rotateX(' + (elm$core$String$fromFloat(x.a6) + (x.bb + (') rotateY(' + (elm$core$String$fromFloat(y.a6) + (y.bb + (') rotateZ(' + (elm$core$String$fromFloat(z.a6) + (z.bb + ')'))))))));
	} else {
		return '';
	}
};
var mdgriffith$elm_style_animation$Animation$Render$renderValues = function (_n0) {
	var model = _n0;
	var _n1 = A2(elm$core$List$partition, mdgriffith$elm_style_animation$Animation$Render$isAttr, model.r);
	var attrProps = _n1.a;
				...
				_List_fromArray(
					[
						...
						_List_fromArray(
							[
								elm$html$Html$text(
								'Inner: ' + elm$core$String$fromFloat(model.z))
							])),
						A2(
						elm$html$Html$input,
						_List_fromArray(
						...
						_List_fromArray(
							[
								elm$html$Html$text(
								'Outer: ' + elm$core$String$fromFloat(model.B))
							])),
						A2(
						elm$html$Html$input,
						_List_fromArray(
						...
						_List_fromArray(
							[
								elm$html$Html$Events$onClick(author$project$Main$FadeLines)
							]),
						_List_fromArray(
							[
								elm$html$Html$text('Fade Out Lines')
							])),
						A2(
						elm$html$Html$div,
						_List_Nil,
						_List_fromArray(
							[
								elm$html$Html$text(
								elm$core$String$fromFloat(model.B) + ('(sin/cos/tan)' + ('(' + (elm$core$String$fromFloat(model.z) + 'x)'))))
							]))
					])),
				A2(
				elm$svg$Svg$svg,
				...
				_List_fromArray(
					[
						...
						_Utils_ap(
							...
							mdgriffith$elm_style_animation$Animation$render(model.r)),
						_List_Nil)
					]))
			]));
};
...
var author$project$Main$main = elm$browser$Browser$element(
	{a0: author$project$Main$init, a8: author$project$Main$subscriptions, bc: author$project$Main$update, bf: author$project$Main$view});
